...
// Winsock for network byte order functions like htonl
...
// Структура для элемента словаря
typedef struct {
    ULONG key;   // Ключ (тип ULONG)
    INT64 value; // Значение (тип INT64)
} DictionaryItem;

// Структура для словаря
typedef struct {
    DictionaryItem* items; // Массив элементов
